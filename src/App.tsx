...
const gltfUrls = [
  'https://raw.githubusercontent.com/cecipizales/MSG-2/main/1.glb?v=2',
  'https://raw.githubusercontent.com/cecipizales/MSG-2/main/2.glb?v=2',
  'https://raw.githubusercontent.com/cecipizales/MSG-2/main/3.glb?v=2'
];
...
function Mosaic({ resolution, iconScale, colorFilter, materialType }: { resolution: number, iconScale: number, colorFilter: string, materialType: string }) {
  ...
  useEffect(() => {
    ...
    // Mosaic dimensions (approximate based on 80 width and 0.75 aspect ratio)
    const mosaicWidth = 80;
    const mosaicHeight = 60; 
    
    // Calculate max Z distance to ensure the mosaic covers the screen completely (FILL)
    const zFitWidth = (mosaicWidth / 2) / (tanHalfFov * aspect);
    const zFitHeight = (mosaicHeight / 2) / tanHalfFov;
    
    // To ensure coverage, we must satisfy both constraints, so take the minimum distance
    const z = Math.min(zFitWidth, zFitHeight);
  ...
  const GRID_ROWS = Math.floor(resolution * 0.75);
  ...
  useFrame(({ clock }) => {
    ...
    const mosaicWidth = 80;
    const spacing = mosaicWidth / GRID_COLS;
    ...
    for (let y = 0; y < GRID_ROWS; y++) {
      for (let x = 0; x < GRID_COLS; x++) {
        ...
        // Position
        // Flip X so it acts like a mirror
        const posX = -(x - GRID_COLS / 2) * spacing + displacements[idx3 + 0]; 
        const posY = -(y - GRID_ROWS / 2) * spacing + displacements[idx3 + 1];
...
function WelcomeScreen({ onLaunch }: { onLaunch: () => void }) {
  return (
    <div className="w-full h-screen bg-[#0A092B] text-white overflow-hidden relative font-sans selection:bg-pink-500/30">
      {/* Floating Background Shapes */}
      <div className="absolute inset-0 pointer-events-none z-0">
        {/* Orange Shape - Top Center/Right */}
        <motion.img 
          src="https://raw.githubusercontent.com/cecipizales/MSG-2/main/orange.png"
          alt="Abstract Orange Shape"
          className="absolute top-[-5%] -right-[10%] md:right-[20%] w-[40vh] md:w-[45vh] object-contain opacity-90"
          animate={{ 
            y: [0, -30, 0],
            rotate: [0, 10, 0],
            scale: [1, 1.05, 1]
        ...
        {/* Pink Shape - Bottom Right (Large) */}
        <motion.img 
          src="https://raw.githubusercontent.com/cecipizales/MSG-2/main/pink.png"
          alt="Abstract Pink Shape"
          className="absolute bottom-[-10%] -right-[20%] md:right-[-10%] w-[60vh] md:w-[90vh] object-contain opacity-90"
          animate={{ 
            y: [0, -20, 0],
            rotate: [0, 5, 0],
          }}
        ...
        {/* Green Shape - Top Right - Moved after Pink to be on top */}
        <motion.img 
          src="https://raw.githubusercontent.com/cecipizales/MSG-2/main/green.png"
          alt="Abstract Green Shape"
          className="absolute top-[2%] -right-[10%] md:right-[2%] w-[25vh] md:w-[30vh] object-contain opacity-90"
          animate={{ 
            y: [0, 40, 0],
            rotate: [0, -15, 0],
          }}
      ...
      {/* Content Container */}
      <div className="relative z-10 h-full flex flex-col justify-center max-w-7xl mx-auto px-6 md:px-16">
        <div className="max-w-4xl">
          <motion.h1 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, ease: "easeOut" }}
            className="text-6xl sm:text-7xl md:text-8xl lg:text-[10rem] font-display font-bold tracking-tighter leading-[0.85] mb-6 md:mb-8 uppercase"
          >
            Interactive<br />Wall
          </motion.h1>
          ...
          <motion.div 
            ...
            className="w-full md:w-2/3 h-px bg-white/40 mb-8 origin-left" 
          />
          ...
          <motion.p 
            ...
            className="text-lg sm:text-xl md:text-3xl font-sans font-light leading-snug opacity-90 mb-10 md:mb-12 max-w-xl"
          >
          ...
          >
            <button 
              onClick={onLaunch}
              className="group relative px-8 py-4 bg-white text-[#0A092B] rounded-full overflow-hidden transition-all hover:shadow-[0_0_40px_rgba(255,255,255,0.3)] hover:scale-105 active:scale-95"
            >
              <div className="flex items-center gap-3 relative z-10">
                <span className="font-bold text-lg tracking-wider uppercase">Launch Demo</span>
                <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </div>
            </button>
            
            <p className="text-[10px] md:text-sm text-white/40 font-mono tracking-wide uppercase pl-2">
              * Enable camera access for interaction
            </p>
          </motion.div>
        </div>
      </div>
...
export default function App() {
  ...
  if (!hasLaunched) {
    return <WelcomeScreen onLaunch={() => setHasLaunched(true)} />;
  }
  ...
  return (
    <div className="w-full h-screen overflow-hidden relative font-sans" style={{ backgroundColor: bgColor }}>
      ...
      {/* Settings button hidden, access via Shift+5 */}
